...
/**
 * This script generates audio files for Moroccan Arabic text content from story JSON files
 * using the PlayHT API. It processes stories and adds paths to the audio files
 * in the JSON data structure.
 * 
 * Usage:
 * - node generate_audio_moroccan.js                    # Processes Moroccan fiction stories (default)
 * - node generate_audio_moroccan.js nonfiction         # Processes Moroccan non-fiction stories
 * 
 * Prerequisites:
...
const storiesFilePath = path.join(
  ...
  'stories_json',
  'moroccan',
  isNonfiction ? 'moroccan_stories_nonfiction.json' : 'moroccan_stories.json'
);
...
console.log(`Processing Moroccan ${isNonfiction ? 'nonfiction' : 'fiction'} stories from: ${storiesFilePath}`);
...
async function processStories() {
  try {
    ...
    for (let i = 0; i < totalStories; i++) {
      ...
      if (isNonfiction) {
        maleAudioField = 'audio_moroccan_nonfiction_male';
        femaleAudioField = 'audio_moroccan_nonfiction_female';
      } else {
        maleAudioField = 'audio_moroccan_male';
        femaleAudioField = 'audio_moroccan_female';
      }
      ...
      const maleAudioFileName = `${storyId}_moroccan_${isNonfiction ? 'nonfiction_' : ''}male.mp3`;
      const femaleAudioFileName = `${storyId}_moroccan_${isNonfiction ? 'nonfiction_' : ''}female.mp3`;
      
      // Set relative path for audio files
      const relativePath = isNonfiction ? 'data/audio/moroccan/nonfiction/' : 'data/audio/moroccan/';
