...
def count_paragraphs(text):
    ...
    if not text:
        return 0
    # Count paragraphs by looking for double newlines or Arabic paragraph separators
...
def split_text_into_paragraphs(text, min_paragraphs=2):
    ...
    # Try to split on common sentence endings in Arabic
    sentences = re.split(r'([.؟!،]\s+)', text)
    if len(sentences) < 3:  # Need at least one split point
        # If can't split by punctuation, split by length
    ...
    if len(combined) == 1:
        # Fallback to splitting by length if we couldn't get sentences
        return split_text_into_paragraphs(text, min_paragraphs)
...
def process_story_file(file_path):
    """Process a story JSON file to ensure paragraph consistency."""
    ...
    for story in data.get('stories', []):
        # Check content_ar and content_en
        ar_content = story.get('content_ar', '')
        en_content = story.get('content_en', '')
        
        # For MSA, use story_content instead of content_ar if it exists
        ...
        # Check if paragraph counts don't match or there's only one paragraph
        if ar_paragraphs != en_paragraphs or ar_paragraphs < 2:
            print(f"  - Fixing '{story.get('title_en', 'Untitled')}': {ar_paragraphs} Arabic vs {en_paragraphs} English paragraphs")
