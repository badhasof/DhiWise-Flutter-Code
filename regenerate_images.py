...
def generate():
    ...
    print(f"Total existing fiction images found: {len(existing_images)}")
    ...
    for story in stories:
        ...
        # Improved prompt for better thumbnail images without text
        prompt = f"Create a generic, artistic illustration related to the story theme '{title_en}'. The image should be simple, minimalist, and visually appealing. DO NOT include any text or words in the image. Use colors and abstract imagery to convey the general mood rather than detailed scenes. Make the imagery generic enough that it could apply to various interpretations of the title."
    ...
    nonfiction_stories = get_nonfiction_story_titles()
    print(f"Total nonfiction stories found: {len(nonfiction_stories)}")
    
    # Check for already generated nonfiction images
    existing_nonfiction_images = set()
    if os.path.exists("assets/nonfiction_images"):
        for file in os.listdir("assets/nonfiction_images"):
            name = os.path.splitext(file)[0]  # Get filename without extension
            existing_nonfiction_images.add(name)
    print(f"Total existing nonfiction images found: {len(existing_nonfiction_images)}")
    
    for story in nonfiction_stories:
        story_id = story['id']
        title_en = story['title_en']
        
        # Skip if this image already exists
        if story_id in existing_nonfiction_images:
            print(f"Skipping {title_en} - image already exists")
            continue
        ...
        # Improved prompt for better thumbnail images without text, adapted for nonfiction
        prompt = f"Create a generic, artistic illustration related to the nonfiction topic '{title_en}'. The image should be simple, minimalist, and visually appealing. DO NOT include any text, words, or labels in the image. Use colors and abstract symbolism to represent the concept rather than detailed literal scenes. Make the imagery generic enough that it could apply to various interpretations of the topic."
